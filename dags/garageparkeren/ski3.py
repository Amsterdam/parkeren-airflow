...
with DAG(
    ...
    staging_to_his = generate_job(
        f"ski3-sta-to-his-{timestamp_str}"[:MAX_JOB_NAME_LENGTH].rstrip("-"),
        NAMESPACE,
        IMAGE,
    ...
    run_staging_to_his = JobOperator(job=staging_to_his, task_id="run-ski3-sta-to-his")

    watch_staging_to_his: BaseOperator = JobSensor(
        job_name=staging_to_his.metadata.name,
        task_id="watch-ski3-sta-to-his",
        namespace=NAMESPACE,
    ...
    to_integration_jobs = [
        ...
        SparkJob(
            job="ski3-his-to-int-telling",
            spark_driver_memory_gb=1,
            spark_executor_memory_gb=1,
            spark_executor_instances=1,
            python_path="/app/src/jobs/historic_to_integration/ski3/telling.py",
            spark_executor_cores=1,
        ),
        SparkJob(
