...
with DAG(
    ...
    for job in range(1):
        test_job = generate_job(
            job_name=f"ski2-migrate-spark-job-{job}-{timestamp_str}"[:MAX_JOB_NAME_LENGTH].rstrip(
                "-"
            ),
            namespace=NAMESPACE,
            image=IMAGE,
            job_script_path="/app/src/jobs/migration/ski2/migrate.py",
        ...
        run_test_job = JobOperator(job=test_job, task_id=f"run-ski2-migrate-spark-job-thomas-{job}")

        watch_test_job: BaseOperator = JobSensor(
            job_name=test_job.metadata.name,
            task_id=f"watch-ski2-migrate-spark-job-{job}",
            namespace=NAMESPACE,
        )
